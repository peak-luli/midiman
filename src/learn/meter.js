...
//
// The number is the hit rate (how you are doing). The fill is how far through
// the pass — due/total — so the bar keeps moving after the opening notes.
...
export function slotStates(ch, n, { results = [], live = null, win = null, done = false } = {}) {
  ...
  results.forEach((r, i) => {
    ...
    out[i] = {
      ...
      width: pct(r.accuracy ?? 0),
      text: (r.ok ? '✓ ' : '✗ ') + pct(r.accuracy ?? 0),
    };
  });
  ...
  // number = hit rate; fill = how far through (due/total). Without total, fall
  // back to the hit rate so a unit test that only sent pct still holds.
...
export function makeMeter(el) {
  ...
  return {
     ...
     * win:     windowStats() for a window challenge.
     *
     * An update that omits `live` still marks the current slot as the active
     * pass (#46). It does not wipe a slot that is already filling (#48).
     */
    update(arg = {}) {
      if (!ch || ch.kind === 'none') return;
      const next = slotStates(ch, slots.length, arg);
      for (const [i, s] of next.entries()) {
        const cur = slots[i];
        if (arg.live == null && arg.win == null && cur
            && /\blive\b/.test(cur.el.className) && s.cls.includes('live') && s.text === '–'
            && cur.val.textContent !== '–') {
          continue;
        }
