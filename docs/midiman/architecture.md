...
- **The snapshot says `paused`, not only `running`.** A pause is a stop somebody
  means to undo, and the phone has to tell the two apart: held, it draws ▶ Resume and
  a ■ Stop and leaves the idle plate off the music the pianist paused to read.
