...
- **Ishay Approved merge** — GitHub Action (5-minute cron sweep) is the **primary** squash-merge when Midiman Dev Status is **Ishay Approved**. Setup: [`ishay-approved-merge.md`](ishay-approved-merge.md). Noa’s half-hour pulse is the stale backup: squash-merge if still Approved ~30m+ and the PR is CLEAN; ping only if blocked.
