...
**Ishay Approved → Done:** a GitHub Action squash-merges when Status is **Ishay Approved** — [`ishay-approved-merge.md`](ishay-approved-merge.md) (secret `MIDIMAN_GITHUB_TOKEN`). Instant wake is org Projects webhook → `repository_dispatch`; cron is backup. Noa’s half-hour pulse is the stale backup: merge if still Approved ~30m+ and the PR is CLEAN; ping only if blocked. After any merge to `main`, [post-merge hygiene](post-merge-hygiene.md) sets **Done**, clears Agent session, and closes leftover open issues.
