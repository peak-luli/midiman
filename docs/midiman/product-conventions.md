...
**Ishay Approved → Done:** a GitHub Action (5-minute cron sweep) is the primary squash-merge — [`ishay-approved-merge.md`](ishay-approved-merge.md). Noa’s half-hour pulse is the stale backup: merge if still Approved ~30m+ and the PR is CLEAN; ping only if blocked.
